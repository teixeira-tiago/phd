...
class Verilog:
    ...
    def generate(self):
        ...
        arquivo.append('\n')
        arquivo.extend(self.sipo([args['pattern'], args['input']]))
        arquivo.append('\n')
        arquivo.extend(self.mux([args['pattern'], args['input']]))
        arquivo.append('\n')
        arquivo.extend(self.hs([args['pattern'], args['input'], args['bitsH'], args['coefficient'], args['H.T']]))
        arquivo.append('\n')
        if args['algo'] == 'GD':
    ...
    def head(self, args):
        ...
        line.append('\twire               en;\n')
        line.append('\twire signed [' + str(args[1]) + ':0] k_sig [' + str(window - 1) + ':0];\n')
        line.append('\twire signed [' + str(args[1] + args[2]) + ':0] w_hs  [' + util.sstr(b - 1) + ':0];\n')
        ...
        if (window <= bwindow):
            ...
            for i in range(abs(halfE), window - halfD):
                aux += 'k_sig[' + util.sstr(i) + '], '
        # aux = aux[:math.ceil(len(aux) / 2)] + '\n\t\t' + aux[len(aux) - math.ceil(len(aux) / 2):]
        line.append('\t\t' + aux + '\n')
        aux = ''
        for cont in range(b):
            aux += 'w_hs[' + util.sstr(cont) + '], '
        line.append('\t\t' + aux[:-2] + ');\n\n')
    ...
    def sipo(self, args):
        ...
        aux = ''
        for cont in range(window):
            aux += '\n\toutput signed [' + str(args[1]) + ':0] out' + util.sstr(cont, '0') + ','
        line.append(aux[:-1] + '\n);\n\n')
        line.append('\treg signed [' + str(args[1]) + ':0] r [' + str(window - 2) + ':0];\n')
        line.append('\talways @ (posedge clk)\n')
        line.append('\tbegin\n')
        line.append('\t\tr[ 0] <= x;\n')
        for cont in range(1, window - 1):
            line.append('\t\tr[' + util.sstr(cont) + '] <= r[' + util.sstr(cont - 1) + '];\n')
        line.append('\tend\n\n')
        line.append('\treg signed [' + str(args[1]) + ':0] r1 [' + str(window - 1) + ':0];\n')
        line.append('\treg [' + util.sstr(math.trunc(math.log(window, 2)) + 1) + ':0] cont = 0;\n\n')
        ...
        line.append('\t\tbegin\n')
        line.append('\t\t\tr1[ 0] <= x;\n')
        for cont in range(1, window):
            line.append('\t\t\tr1[' + util.sstr(cont) + '] <= r[' + util.sstr(cont - 1) + '];\n')
        line.append('\t\tend\n')
        line.append('\tend\n\n')
        for cont in range(window):
            line.append('\tassign out' + util.sstr(window - 1 - cont, '0') + ' = r1[' + util.sstr(cont) + '];\n')
        line.append('\nendmodule\n')
        return line
    ...
    def mux(self, args):
        ...
        for cont in range(b):
            line.append('\tinput      signed [' + str(args[1]) + ':0]   a' + util.sstr(cont, '0') + ',\n')
        for cont in range(b):
        ...
        for cont in range(b):
            line.append(
                '\t\t\tc' + util.sstr(cont, '0') + ' <= a' + util.sstr(cont, '0') + ' << gain;\n')
        line.append('\t\tend\n\t\telse\n\t\tbegin\n')
        for cont in range(b):
    ...
    def hs(self, args):
        ...
        for i in range(start, ended):
            line.append('\tinput  signed [' + util.sstr(args[1]) + ':0]  in' + util.sstr(i, '0') + ',\n')
        aux, auxM = '', ''
        for i in range(b):
            aux += '\toutput signed [' + util.sstr(bH + args[1]) + ':0] out' + util.sstr(i, '0') + ',\n'
        line.append(aux[:-2] + '\n);\n\n')
        lineM = ['$Eliminar a linha inteira: .*Conteudo.*\\n?\n$Para eliminar o codigo Python por exemplo usar: .*#.*\\n?\n']
        lineM.append('%function [out] = hsignal(in, h)\n')
        lineM.append('//VectorXd hsignal(VectorXd in, VectorXd h) {\n//\tVectorXd out('+str(b)+');\n')
        lineM.append('#def hsignal(self, inp, h):\n#    out = np.zeros('+str(b)+')\n')
        for i in range(b):
            aux, auxM, auxC, auxP = '', '', '', ''
            for j in range(start, ended):
                if ((ht[i][j] > 0.0) and ((math.ceil(ht[i][j] * math.pow(2, bH - 1)) - 1) > 0)):
                    index = coef.tolist().index(ht[i][j])
                    aux += '(h' + util.sstr(index, '0') + ' * in' + util.sstr(j, '0') + ') + '
                    auxM += '(h(' + str(index + 1) + ') * in(' + str(j + 1) + ')) + '
                    auxC += '(h(' + str(index) + ') * in(' + str(j) + ')) + '
                    auxP += '(h[' + str(index) + '] * inp[' + str(j) + ']) + '
            line.append('\tassign out' + util.sstr(i, '0') + ' = ' + aux[:-3] + ';\n')
            lineM.append('%\tout(' + str(i + 1) + ') = ' + auxM[:-3] + ';\n')
            lineM.append('//\tout(' + str(i) + ') = ' + auxC[:-3] + ';\n')
            lineM.append('#    out[' + str(i) + '] = ' + auxP[:-3] + '\n')
        line.append('\nendmodule\n')
        lineM.append('%end\n')
        lineM.append('//\treturn out;\n//}\n')
        lineM.append('#    return out')
        file = open('../results/hsignal.m', 'w')
        for linha in lineM:
            file.write(linha)
        file.close()
        return line
    ...
    def ax(self, args):
        ...
        lineM.append('\n')
        for cont in range(b):
            line.append('\tassign B[' + util.sstr(cont) + '] = (hs' + util.sstr(cont, '0') + ' << (bits - 5)) - Ax[' + util.sstr(
                cont) + '];\n')
            lineM.append('%\tB(' + str(cont + 1) + ') = (hs(' + str(cont + 1) + ') * (2 ^ (bits - 5))) - Ax(' + str(
                cont + 1) + ');\n')
            lineM.append('//\tB(' + str(cont) + ') = (hs(' + str(cont) + ') * pow(2, (bits - 5))) - Ax(' + str(cont) + ');\n')
            lineM.append('#    B[' + str(cont) + '] = (hs[' + str(cont) + '] * pow(2, (bits - 5))) - Ax[' + str(cont) + ']\n')
        line.append('\n')
        lineM.append('\n')
        return [line, lineM]
    ...
    def gdp(self, args):
        ...
        line.append('\tparameter bits = 15\n)\n(\n')
        for cont in range(b):
            line.append('\tinput  signed [' + util.sstr(args[1] + args[2]) + ':0]    hs' + util.sstr(cont, '0') + ',\n')
        for cont in range(b):
        ...
        lineM = ['%function [out] = gdp(hs, in, bits, a, align, mu)\n%\ttmp = zeros(1, ' + str(
            b) + ');\n%\tAx = zeros(1, ' + str(b) + ');\n%\tB = zeros(1, ' + str(b) + ');\n%\txB = zeros(1, ' + str(
            b) + ');\n\n']
        lineM.append(
            '//VectorXd gdp(VectorXd hs, VectorXd in, int bits, VectorXd a, int align, double mu) {\n//\tVectorXd tmp(' + str(
                b) + ');\n//\tVectorXd Ax(' + str(b) + ');\n//\tVectorXd B(' + str(b) + ');\n//\tVectorXd xB(' + str(
                b) + ');\n//\tVectorXd out(' + str(b) + ');\n\n')
        lineM.append('#def gdp(self, hs, inp, bits, a, align, mu):\n#    tmp = np.zeros(' + str(
            b) + ')\n#    Ax = np.zeros(' + str(b) + ')\n#    B = np.zeros(' + str(b) + ')\n#    xB = np.zeros(' + str(
            b) + ')\n#    out = np.zeros(' + str(b) + ')\n\n')
    ...
    def gd(self, args):
        ...
        line.append('\twire signed [64:0]   xB [' + util.sstr(b - 1) + ':0];\n\n')
        lineM = [
            '%function [out] = gd(hs, in, bits, a, align, mu)\n%\tAx = zeros(1, ' + str(b) + ');\n%\tB = zeros(1, ' + str(
                b) + ');\n%\txB = zeros(1, ' + str(b) + ');\n\n']
        lineM.append(
            '//VectorXd gd(VectorXd hs, VectorXd in, int bits, VectorXd a, int align, double mu) {\n//\tVectorXd Ax(' + str(
                b) + ');\n//\tVectorXd B(' + str(b) + ');\n//\tVectorXd xB(' + str(b) + ');\n//\tVectorXd out(' + str(
                b) + ');\n\n')
        lineM.append(
            '#def gd(self, hs, inp, bits, a, align, mu):\n#    Ax = np.zeros(' + str(b) + ')\n#    B = np.zeros(' + str(
                b) + ')\n#    xB = np.zeros(' + str(b) + ')\n#    out = np.zeros(' + str(b) + ')\n\n')
        ax = self.ax([b, A, bA, coefA])
        line.extend(ax[0])
        lineM.extend(ax[1])
    ...
    def ssf(self, args):
        ...
        line.append('\twire signed [64:0]    B  [' + util.sstr(b - 1) + ':0];\n')
        line.append('\twire signed [64:0]   xB  [' + util.sstr(b - 1) + ':0];\n\n')
        lineM = ['%function [out] = ssf(hs, in, bits, a, align, gain, mu, lambda)\n%\ttmp = zeros(1, ' + str(
            b) + ');\n%\tAx = zeros(1, ' + str(b) + ');\n%\tB = zeros(1, ' + str(b) + ');\n%\txB = zeros(1, ' + str(
            b) + ');\n\n']
        lineM.append(
            '//VectorXd ssf(VectorXd hs, VectorXd in, int bits, VectorXd a, int align, int gain, double mu, double lambda) {\n//\tVectorXd tmp(' + str(
                b) + ');\n//\tVectorXd Ax(' + str(b) + ');\n//\tVectorXd B(' + str(b) + ');\n//\tVectorXd xB(' + str(
                b) + ');\n//\tVectorXd out(' + str(b) + ');\n\n')
        lineM.append('#def ssf(self, hs, inp, bits, a, align, gain, mu, lamb):\n#    tmp = np.zeros(' + str(
            b) + ')\n#    Ax = np.zeros(' + str(b) + ')\n#    B = np.zeros(' + str(b) + ')\n#    xB = np.zeros(' + str(
            b) + ')\n#    out = np.zeros(' + str(b) + ')\n\n')
        ax = self.ax([b, A, bA, coefA])
        line.extend(ax[0])
    ...
    def pcd(self, args):
        ...
        lineM = ['%function [out] = pcd(hs, in, bits, a, align, gain, mu, lambda, IW)\n%\taux = zeros(1, ' + str(
            b) + ');\n%\tAx = zeros(1, ' + str(b) + ');\n%\taB = zeros(1, ' + str(b) + ');\n%\tB = zeros(1, ' + str(
            b) + ');\n%\txB = zeros(1, ' + str(b) + ');\n%\ttmp = zeros(1, ' + str(b) + ');\n\n']
        lineM.append(
            '//VectorXd pcd(VectorXd hs, VectorXd in, int bits, VectorXd a, int align, int gain, double mu, double lambda, double IW)\n//\tVectorXd aux(' + str(
                b) + ');\n//\tVectorXd Ax(' + str(b) + ');\n//\tVectorXd aB(' + str(b) + ');\n//\tVectorXd B(' + str(
                b) + ');\n//\tVectorXd xB(' + str(b) + ');\n//\tVectorXd tmp(' + str(b) + ');\n//\tVectorXd out(' + str(
                b) + ');\n\n')
        lineM.append('#def pcd(self, hs, inp, bits, a, align, gain, mu, lamb, IW):\n#    aux = np.zeros(' + str(
            b) + ')\n#    Ax = np.zeros(' + str(b) + ')\n#    aB = np.zeros(' + str(b) + ')\n#    B = np.zeros(' + str(
            b) + ')\n#    xB = np.zeros(' + str(b) + ')\n#    tmp = np.zeros(' + str(
            b) + ')\n#    out = np.zeros(' + str(b) + ')\n\n')
        ax = self.ax([b, A, bA, coefA])
    ...
    def tas(self, args):
        ...
        line.append('\twire signed [64:0]    B  [' + util.sstr(b - 1) + ':0];\n')
        line.append('\twire signed [64:0]   tmp [' + util.sstr(b - 1) + ':0];\n\n')
        lineM = ['%function [out] = tas(hs, in, bits, a, align, gain, mu, lambda, t)\n%\taux = zeros(1, ' + str(
            b) + ');\n%\tAx = zeros(1, ' + str(b) + ');\n%\tB = zeros(1, ' + str(b) + ');\n%\txB = zeros(1, ' + str(
            b) + ');\n%\ttmp = zeros(1, ' + str(b) + ');\n\n']
        lineM.append(
            '//VectorXd tas(VectorXd hs, VectorXd in, int bits, VectorXd a, int align, int gain, double mu, double lambda, double t)\n//\tVectorXd aux(' + str(
                b) + ');\n//\tVectorXd Ax(' + str(b) + ');\n//\tVectorXd B(' + str(b) + ');\n//\tVectorXd xB(' + str(
                b) + ');\n//\tVectorXd tmp(' + str(b) + ');\n//\tVectorXd out(' + str(b) + ');\n\n')
        lineM.append('#def tas(self, hs, inp, bits, a, align, gain, mu, lamb, t):\n#    aux = np.zeros(' + str(
            b) + ')\n#    Ax = np.zeros(' + str(b) + ')\n#    B = np.zeros(' + str(b) + ')\n#    xB = np.zeros(' + str(
            b) + ')\n#    tmp = np.zeros(' + str(b) + ')\n#    out = np.zeros(' + str(b) + ')\n\n')
        ax = self.ax([b, A, bA, coefA])
        line.extend(ax[0])
        ...
        for ite in range(samples):
            ...
            for i in range(iteration):
                if args[1] == 'GD':
                    x = algo.GD(x, Hs, A, mu)
                if args[1] == 'GDP':
                    x = algo.GDP(x, Hs, A, mu)
                if args[1] == 'SSF':
                    x = algo.SSF(x, Hs, A, mu, args[5])
                if args[1] == 'PCD':
                    x = algo.PCD(x, Hs, A, mu, args[5], const)
                if args[1] == 'TAS':
                    x = algo.TAS(x, Hs, A, mu, args[5], const)
            x = np.where(x < 0, 0, x)
